    ...
    for i in range(board_size):
        for j in range(board_size):
            if population[i][j][0] == 4:
                ...
                if state == 1:
                    if not (i > 0 and j > 0 and population[i - 1][j - 1][0] == 1) and \
                            not (i > 0 and population[i - 1][j][0] == 1) and \
                            not (i > 0 and j < board_size - 1 and population[i - 1][j + 1][0] == 1) and \
                            not (j > 0 and population[i][j - 1][0] == 1):
...
def paintNdraw(i, j, color, state):
    fontSize = 5
    padding = 5
    canvas.create_rectangle(j * rectangle_size, i * rectangle_size, (j + 1) * rectangle_size,
                            (i + 1) * rectangle_size,
                ...
                for x in [-1, 0, 1]:
                    for y in [-1, 0, 1]:
                        ...
                        if neighbor_x >= 0 and neighbor_x < board_size and neighbor_y >= 0 and neighbor_y < board_size:
                            ...
                            if population[neighbor_x][neighbor_y][1] == "unbeliever" and L_matrix[neighbor_x][
                                neighbor_y] == 0:
                                ...
                                if neighbor_type == 1:
                                    ...
                                    if random() < 2 / 3:
                                        updated_population[neighbor_x][neighbor_y][1] = "believer"
                                        # Paint cell red
                                        paintNdraw(neighbor_x, neighbor_y, 'red',
                                                   population[neighbor_x][neighbor_y][0])
...
while valid:
    ...
    fields = ["P (Percentage of settlers in the population) ", "L (#generation without receiving a rumor)", "s1",
              "s2", "s3", "s4"]
    default_values = ["0.75", "2", "0.15", "0.35", "0.35", "0.15"]
